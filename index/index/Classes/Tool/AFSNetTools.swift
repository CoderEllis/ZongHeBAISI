...
class AFSNet: NSObject {
    ...
    typealias AFSNetSuccessBlock = ([String: Any],SwiftyJSON.JSON) -> ()
    
    typealias AFSNetFaliedBlock = (AFSErrorInfo) -> ()
    
    typealias AFSProgressBlock  = (Double) -> ()
    ...
    func post(url: String, param: Parameters?, successBlock: @escaping AFSNetSuccessBlock, faliedBlock: @escaping AFSNetFaliedBlock) {
        ...
        requestData(.post, urlString: url, parameters: param, successBlock: successBlock, faliedBlock: faliedBlock)
    }
    ...
    func get(url: String, successBlock: @escaping AFSNetSuccessBlock, faliedBlock: @escaping AFSNetFaliedBlock) {
        ...
        requestData(.get, urlString: url, parameters: nil, successBlock: successBlock, faliedBlock: faliedBlock)
    }
    
    private func requestData(_ type: MethodType, urlString: String, parameters: Parameters?, successBlock: @escaping AFSNetSuccessBlock, faliedBlock: @escaping AFSNetFaliedBlock) {
    ...
    private func handleResponse( response: DataResponse<Any>, successBlock:AFSNetSuccessBlock, faliedBlock:AFSNetFaliedBlock) {
        ...
        }else if let jsonDic = response.result.value { // 服务器又返回数据
            
            if (jsonDic as? [String: Any]) == nil { // 返回格式不对
                
                self.handleRequestSuccessWithFaliedBlcok(faliedBlock: faliedBlock)
                
            }else{
                
                self.handleRequestSuccess(value: jsonDic, successBlock: successBlock, faliedBlock: faliedBlock)
        ...
        case .success(let value): 
            ...
            if (value as? [String: Any]) == nil {
                ...
                handleRequestSuccessWithFaliedBlcok(faliedBlock: faliedBlock)
                
            } else {
                
                handleRequestSuccess(value: value, successBlock: successBlock, faliedBlock: faliedBlock)
            }
            
        case .failure(let error):
            handleRequestError(error: error as NSError, faliedBlock: faliedBlock)
    ...
    private func handleRequestSuccess(value: Any, successBlock: AFSNetSuccessBlock, faliedBlock:AFSNetFaliedBlock) {
        ...
        if json["code"].int != nil && json["code"].int! == 1 { // 拦截
            ...
            successBlock(value as! [String: Any],json)
